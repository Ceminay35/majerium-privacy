...
Version: 1.0.0
Akinator benzeri yer tahmin oyunu
...
ORBI_COMMENTS = {
    "start": [
        ...
        "Hazır mısın? Dünyanın neresini saklıyorsun? 🧭",
        "Keşfe başlayalım! Aklındaki yeri söylemeden ben bulayım. 🔍"
    ],
    "high_candidates": [
        "Hmm... Henüz çok fazla ihtimal var. 🤔",
        "Daha başlangıçtayız, biraz sabır... 🧠",
        "Adaylar arasında kayboldum, biraz ipucu ver! 😅"
    ],
    "medium_candidates": [
        "İşler netleşmeye başladı! 🎯",
        "Gidiyoruz! Aday sayısı azalıyor. 📉",
        "İpucu peşindeyim, doğru yoldayım! 🔦"
    ],
    "low_candidates": [
        "Çok yaklaştım! Son birkaç aday kaldı. 🎪",
        "Bence artık biliyorum! 🕵️",
    ...
    "not_found": [
        "Bilemedim... Bu yer benim veritabanımda yok. 😢",
        "Seni yendin! Orbi bugün kaybetti. 💔",
        "Keşke bu yeri bilseydim... Öğretir misin? 📚"
    ]
}
...
def get_all_questions() -> List[Dict]:
    ...
    # Boolean özelliklerden dinamik sorular
    boolean_props = {
...
def calculate_information_gain(landmarks: List[Dict], question: Dict) -> float:
    ...
    if question["type"] == "boolean":
        ...
        # Bilgi kazancı = entropy * (1 - (true_count/total)^2 - (false_count/total)^2)
...
def get_best_question(landmarks: List[Dict], asked_questions: List[str]) -> Optional[Dict]:
    ...
    for q in all_questions:
        ...
        # Rastgelelik ekle (aynı gain'de farklı sorular çıksın)
...
def main():
    ...
    if not st.session_state.game_started:
        ...
        with st.expander("📌 Örnek Yerler"):
            sample_places = [
                "Ayasofya", "Pamukkale", "Kapadokya", "Göbeklitepe",
                "Burj Khalifa", "Eyfel Kulesi", "Piramitler", "Taj Mahal"
            ]
            cols = st.columns(4)
    ...
    st.markdown(f"""
    <div class="footer">
        <p>🌍 ORBI DISCOVERY ENGINE v1.0</p>
